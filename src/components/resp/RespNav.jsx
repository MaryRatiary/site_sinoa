...
export default function RespNav() {
    ...
    const [isCartOpen, setIsCartOpen] = useState(false); // ✅ NOUVEAU - state du modal
    const [categories, setCategories] = useState([]);
    const [loading, setLoading] = useState(true);
    const [isSearching, setIsSearching] = useState(false);
    
    const navigate = useNavigate();
    const { cartItems } = useCart(); // ✅ MODIFIÉ - utiliser cartItems au lieu de getItemCount
    ...
    return (
        <>
            <div className="lg:hidden bg-white">
                ...
                <div className="flex justify-between items-center px-4 py-3 border-b border-gray-200 gap-2 min-h-[64px]">
                    {!isSearching && (
                        <button 
                    ...
                    <div className={`flex items-center gap-2 ${isSearching ? "flex-1 transition-all duration-300" : ""}`}>
                        <ExpandSearch onToggle={setIsSearching} />
                        ...
                        {!isSearching && (
                            <button 
                                ...
                                className="relative p-2 hover:bg-gray-100 rounded-lg transition hover:text-[#5E2251]">
                                <ShoppingBag size={20} />
                                {cartItems.length > 0 && (
