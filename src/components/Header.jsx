...
const Navbar = () => {
  ...
  return (
    <nav className="relative w-full bg-white border-b border-gray-100">
      ...
      {/* Dropdown Shop */}
      {activeMenu === 'shop' && (
        <div className="absolute top-full left-0 w-full bg-white z-50 shadow-lg border-t-2 border-[#5E2251] p-10 grid grid-cols-5 gap-8" {...dropdownProps}>
          <div className="col-span-">
            {dropdownData.shop.featured.map((f) => (
              <a
                key={f.label}
                href={f.url}
                className={`relative rounded-lg overflow-hidden aspect-video flex items-end px-3 text-white font-bold ${f.italic ? 'italic' : ''} text-sm group`}
              >
                {/* Image */}
                <img
                  src={f.image}
          ...
          {dropdownData.shop.columns.map((col) => (
            <MenuColumn key={col.title} title={col.title} items={col.items} />
          ))}
        </div>
      )}
