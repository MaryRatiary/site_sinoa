...
export const RichDescriptionEditor = ({ value, onChange, placeholder = "Décrivez votre produit..." }) => {
  ...
  const markdownHelp = [
    ...
    { syntax: '| Col1 | Col2 |', example: 'Tableau' },
  ];
  ...
  return (
    <div className="space-y-2">
      ...
      {!showPreview ? (
        ...
        <div className="w-full min-h-[300px] p-4 border border-gray-300 rounded-lg bg-white prose prose-sm max-w-none overflow-auto">
          <ReactMarkdown
            ...
            components={{
              ...
              thead: ({ node, ...props }) => (
                <thead className="bg-gradient-to-r from-purple-50 to-purple-100 border-b-2 border-purple-200" {...props} />
              ),
              tbody: ({ node, ...props }) => (
                <tbody {...props} />
              ),
              tr: ({ node, index, ...props }) => (
                <tr className={index % 2 === 0 ? 'bg-white hover:bg-purple-50' : 'bg-gray-50 hover:bg-purple-50'} {...props} />
              ),
              th: ({ node, ...props }) => (
                <th className="px-4 py-2 text-left font-bold text-gray-900 border-b border-purple-200" {...props} />
              ),
              td: ({ node, ...props }) => (
                <td className="px-4 py-2 text-gray-700 border-b border-gray-100 text-sm" {...props} />
              ),
            }}
          >
            {value}
