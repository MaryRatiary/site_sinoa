...
export default function ShopPage() {
  ...
  const staticProducts = useMemo(() => [
    ...
    ...huntrix,
    ...lightStick,
    ...fashion.map(p => ({ ...p, name: p.name || p.title })),
    ...beauty.map(p => ({ ...p, name: p.name || p.title })),
  ].map(p => ({ ...p, _source: 'static' })), []);

  const normalizedDbProducts = useMemo(() =>
    dbProducts.map(p => ({ ...p, _source: 'db' })), [dbProducts]);

  const allProducts = useMemo(() => {
    const slugsSeen = new Set();
    const merged = [...staticProducts, ...normalizedDbProducts].filter(p => {
      const key = p.slug || p.id;
      if (!key || slugsSeen.has(key)) return false;
      slugsSeen.add(key);
      return true;
    });
    return shuffleArray(merged);
  }, [staticProducts, normalizedDbProducts]);

  const sortedProducts = useMemo(() => {
    setCurrentPage(1); // reset page quand on change le tri
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-4 md:gap-6">
            {paginatedProducts.map((product, idx) => (
              product._source === 'db' ? (
                ...
                <ProductCard
                  key={`static-${product.id || product.slug}-${idx}`}
                  id={product.id}
                  slug={product.slug}
