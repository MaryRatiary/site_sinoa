  ...
  useEffect(() => {
    const fetchBestsellers = async () => {
      try {
        ...
        // Filtrer les bestsellers par catégories
        const bestsellers = allProducts.filter(p => {
          return Object.values(CATEGORIES).flat().includes(p.category_name);
        });

        setDbProducts(bestsellers);
      } catch (error) {
      ...
      <div className="w-full bg-white py-3 sm:py-4 md:py-8 px-3 sm:px-4">
        <div className="max-w-7xl mx-auto flex flex-col items-center text-center gap-2">
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-[10px] font-bold uppercase tracking-[0.3em] text-[#5E2251]">Elite Equipment</span>
          </div>
          <h1 className="text-2xl sm:text-3xl md:text-5xl font-black text-gray-900 tracking-tight">
            <span className="italic text-secondary-500" style={{color: '#001c66'}}>Best sellers</span> 
            <span className="italic ml-2" style={{color: '#8318b3'}}>du moment</span>
          </h1>
          {!dbLoading && (
            <p className="text-xs sm:text-sm text-gray-400 mt-1">{sortedProducts.length} articles disponibles</p>
          )}
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-4 pb-8">
        {dbLoading ? (
          ...
          <div className="text-center py-16">
            <p className="text-gray-400 text-sm">Aucun bestseller trouvé</p>
          </div>
        )}
      </div>
